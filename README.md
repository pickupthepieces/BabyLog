...
> `app/`（TS web 原型）已冻结归档，主线为 `android-native/`。
...
| 之后 | PWA（跨端访问）、同步体验继续打磨 | 家庭密钥 E2EE push/pull 和附件文件加密同步已接入 |
...
| 同步 | PocketBase 家庭密钥 E2EE push/pull + 附件文件加密同步；服务端只见密文，未配置时纯本机可用 |
