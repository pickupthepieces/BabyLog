...
**给一个家庭，把从备孕、孕期到宝宝慢慢长大的点点滴滴，安安心心地记在自己手机里。**
...
[![Stage](https://img.shields.io/badge/scope-孕期主线-E8896B.svg)](#成长路线)

一家人——爸爸妈妈、爷爷奶奶、外公外婆，必要时还有月嫂——一起，从备孕到孕期，
再到宝宝出生慢慢长大，把这一路都记下来。它的脾气一直没变：
**数据留在自己手机里、不用联网也能用、和医疗有关的只帮忙记，不替你下结论。**
...
别的记录工具，要么把家里的事存到别人的服务器上，要么把"参考范围"说得像医生的结论。
BabyLog 反过来：

- **东西是你家自己的**　不用注册、不用联网也能完整用，不依赖任何后端。
- **就认真陪一个宝宝**　数据结构给多宝宝留了位置，但界面只服务一家这一个清楚的场景。
- **孕期是孕期，出生了是出生后**　同一条时间线，会按阶段自动换成该看的样子。
- **机器只是帮着认字**　B 超单识别、语音转写、智能解析都只先给个候选，**你点头确认了才算数**。

## 能帮着记些什么

### 孕期（现在的主线，已经能好好用了）

- **首页一眼看明白**：第几周、离预产期还有几天、最近的产检和 B 超、有没有要复核的
- **产检 / 胎动 / 宫缩** 一拍就记；宝宝踢肚子时有个胎动计数器（开始 / 计数 / 计时 / 到 10 次提醒）
- **B 超的数字** 都能录（BPD / HC / AC / FL / EFW…），超出常用范围会轻轻提示，单子能拍照存好
- **B 超单拍一下就识别**：接你自己配的多模态模型，只生成候选，要你核对
- **对着手机说一句**：按住说话 → 独立配置的 Paraformer 转成能改的文字 → 自动打开该填的表单
- **画出成长曲线**：按孕周把自家的 B 超趋势画出来
