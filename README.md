...
**一个家庭自用的孕育记录 App —— 本地优先、隐私自持、医疗内容只做记录与参考。**
...
市面上的孕育记录工具，要么把数据放在别人的服务器上，要么把"参考范围"包装成像诊断的结论。
BabyLog 反过来：**记录是你的、留在你手机里**；医疗相关内容**只呈现、不判断**，并在每一处都如实标注其参考性质与不确定性。

- **本地优先**　无需账号、无需联网即可完整使用；不依赖后端。
- **单胎单宝宝**　数据模型预留 1:N，界面只服务一个真实家庭的清晰场景。
- **阶段投影**　同一条时间线，按"孕期 / 出生后"自动切换首页与录入。
- **人在回路**　任何模型识别（B 超 OCR、语音转写、智能解析）只生成候选，**人工确认后才入库**。
...
- **产检 / 胎动 / 宫缩**快捷记录；胎动会话计数器（开始 / 计数 / 计时 / 目标提示）
- **B 超指标**录入（BPD / HC / AC / FL / EFW…）+ 软范围提示 + 拍照归档
- **B 超单识别**：接入用户自带的 OpenAI-compatible 多模态模型，识别仅生成候选字段
- **全局语音录入**：按住说话 → 独立配置的 Paraformer STT 转可编辑文本 → 智能解析打开对应表单
- **胎儿成长曲线**：按孕周绘制自有 B 超趋势
...
| **进行中** | 单 Activity → Navigation-Compose 多页面架构重构 | P0–P4 已完成，P5 收尾 |
