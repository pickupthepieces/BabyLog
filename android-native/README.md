...
- 全局智能录入已收敛为单一入口：用户提交文本后由大模型判类型并打开对应表单预填，确认前不入库；语音 v1 将在同一入口内加入 App 内录音 + 云端 STT。
...
- FGR / 成长曲线计算。
- 账号登录和家庭共享。
- App 内录音语音识别入口。语音 v1 架构已定：按住录音 -> 云端专业 STT（DashScope Paraformer 首选，OpenAI Transcribe 备选）-> 可编辑转写文本 -> 大模型结构化候选 -> 表单确认后入库。
