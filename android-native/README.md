...
BabyLog Android 原生 MVP，优先验证小米真机上的 APK 方向。当前实现使用 Java + AndroidX AppCompat，保留手写原生 View，不引入 Compose 或 CameraX。
...
- AndroidX AppCompat 依赖会由 Gradle 下载；不需要安装额外桌面环境。
