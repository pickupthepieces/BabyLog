...
- 家庭同步：同步设置支持服务器地址和家庭密钥，家庭密钥仅本机加密保存。S3 起家庭密钥查找值改为 HKDF lookup hash，旧裸 SHA-256 hash 会失效，用户需重新填写一次家庭密钥；S5 已支持 E2EE 加密推送、自动拉取、下拉刷新、静默 LWW 合并、防回路，以及附件文件独立加密上传 / 按需下载，服务端仅见密文。
