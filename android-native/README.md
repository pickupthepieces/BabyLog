...
- 家庭同步：本机 pending 队列 + 家庭同步协议骨架；同步设置支持服务器地址和家庭密钥，家庭密钥仅本机加密保存。S3 起家庭密钥查找值改为 HKDF lookup hash，旧裸 SHA-256 hash 会失效，用户需重新填写一次家庭密钥；当前目标为 push only E2EE，服务端仅见密文，拉取和附件跨设备同步留到 S4。
