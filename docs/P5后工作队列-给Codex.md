...
## 深读发现（Claude 读实现体，2026-05-19）：新事件类型的可见性跨切要求（直接影响 Q2b-2）

读 `resolveCareStage`/`isEventVisibleInHome`/`timelineFilterGroup` 实现体确认：

- **无 bug 部分（深读坐实，非速扫）**：`resolveCareStage` 逻辑成立(override>birthDate到>孕期>未知)；`maternal_metric`/`fetal_movement`/`contraction` → group `"pregnancy"`，**在孕期首页可见、无 bug**（此前担心已被实现否定）。
- **🔴 跨切硬要求（Q2b-2 必须同步，否则埋雷）**：`timelineFilterGroup` 默认 `"all"`；`isEventVisibleInHome(STAGE_PREGNANCY)` 仅认 group∈{pregnancy,ultrasound,checkup}。**`screening_*` 7 类未映射 → 落 "all" → 不在该集合 → 孕期首页"最近记录"隐身，时间线仅"全部"档可见、分类档错配**。Q2b-2 必须：① 在 `BabyLogFormatters.timelineFilterGroup` 把 `screening_*` 映射到 `"checkup"`（或新增 screening 分组并同步 `isEventVisibleInHome` 与 `TimelineFilters` 选项）；② 确认新类型在孕期首页可见、时间线分类正确、Q2c 详情/Q2f 编辑均覆盖。
- **通用规则（写入所有"新增 eventType"工单）**：任何新增事件类型，必须同步更新 `timelineFilterGroup` + `isEventVisibleInHome` + `TimelineFilters` + 详情/编辑可达，并装机确认"首页能看到、时间线分类正确、可查看可编辑"——否则即为"看不到自己数据"缺陷。

> 审计方法纠偏：本节经"读实现体"得出（resolveCareStage/isEventVisibleInHome/timelineFilterGroup 全文），非 grep 速判；Q2e 同法已自我降级。剩余全量逐行（FGR 全分支、各表单保存校验、smart entry 全链）建议新会话满用量续审。

## Q2h（中）表单可存空记录：除 B 超外无最小内容校验（Claude 读实现体确认）

**缺陷**：`recordUltrasound` 有 `hasUltrasoundMinimumContent`（照片或≥1生长指标）+ Service 抛异常双重防空——正确，作参考样板。但 `recordMaternalMetric`/`recordPregnancyEvent`(含胎动/宫缩/产检)/`recordBabyCareEvent` **均无防空校验**（只 buildPayload→createEvent→存），对应表单 `onSave` 也直接 build+保存。→ 全空白点"保存"即生成空记录，污染时间线、消耗 Q2d 100 额度、摘要显"今天"却无数据。逻辑不一致。

**做法（一笔 `fix:`，仿 B 超样板）**：为 maternal_metric/pregnancy_checkup/fetal_movement/contraction/babyCare 各加 `hasXxxMinimumContent`（至少一个有意义字段非空），Service `recordXxx`/`updateXxx` 入口校验不过则抛/拒，表单层捕获并 `saveError` 提示（与 Ultrasound 一致体验）。**quick 一拍即记**(奶瓶等，用户既定保留)走的是带默认值的快速路径，不在此约束内，勿误伤——仅约束"表单走完全空保存"。验收：四类表单全空保存被拒并提示；有任一有效字段可存；quick 一拍即记不受影响；编辑(Q2)同样校验。

> 方法纪律：本条经读 `hasUltrasoundMinimumContent` 与三个 `recordXxx` 实现体确认，非速扫。

## Q2i（低/一致性）`daysBetween` 未钉时区，偏离 Asia/Shanghai 纪律（Claude 读体确认）

**确认（已读 CMA `daysBetween` 实现体）**：`SimpleDateFormat("yyyy-MM-dd", Locale.US)` 未 `setTimeZone`，按设备默认 TZ 解析，再 `(to.time-from.time)/86_400_000L` 整除取天。跨 DST 的两午夜差 23h/25h，整除截断 → 孕周/距预产期/日龄/下次产检倒数 off-by-one。中国 Asia/Shanghai 当前无 DST，**本机实测无影响**；但项目自 L-9 已统一钉 `Asia/Shanghai`/`cnFormat`，此处是遗漏的一致性偏离，设备处于有 DST 时区即错。

**做法（一笔 `fix:`，极小）**：`daysBetween` 改用项目既有 `BabyLogFormatters.cnFormat`/Asia/Shanghai 解析（或对 SimpleDateFormat `setTimeZone(Asia/Shanghai)`），与全局日期纪律一致；不改调用方/语义。验收：跨 DST 时区设备下孕周/倒数不偏移；CN 无回归；smoke 绿。低优先，可并入任一 fix 轮顺手做。

---

## 批量评审(Claude,2026-05-20):工单 1→9 + Q2i 全部通过

10 笔提交 `f42ea98..07af6ed` 全部 review 通过,可 push。

- **f42ea98 Q2d**:`listTimelineEvents()` 无 limit 用于 `state.timeline`;`listRecentEvents(20)` 仅 dashboard。数据源解耦,旧记录与曲线史不再隐身。
- **b107a88 Q2e**:`LAST_IMPORT_UNDO_FILE` 一次性快照 + `undoLastImport()` 撤销入口,Settings 复阅入口接好。
- **fd608ae perf-C**:F1 rail/F2 TimelineRow 派生串remember+elevation→0。
- **ef1edfe Q2c**:`RecordDetailScreen.kt` 只读详情 + route;Home/Timeline 点击进入。
- **7733d4f Q2f**:胎动/宫缩/baby 纳入可编辑(Service+UI 同步)。
- **18585de Q2h**:四类表单 `hasXxxMinimumContent` 防空,quick 一拍即记未误伤。
- **92703e9 Q2g**:时间线/资料库本地检索。
- **2f118f0 Q2b-2**:screening_* 7 类结构化;**跨切硬要求达成**——`timelineFilterGroup` line 447 `pregnancy_checkup || isScreeningEventType → "checkup"`,`isEventVisibleInHome` 含 checkup → 孕期首页可见不隐身;`risk*` 字段为透传报告/用户输入非 App 计算 → **零判读底线守住**。
- **7f97a7f Q6**:FGR 文档"现行决策"头 + READMEs 一致性。
- **07af6ed final fix**:① 新增 `daysBetweenDateInputs` 用 `cnFormat`(Asia/Shanghai)→ **Q2i 顺手修了**;② `createEditedEvent` 重载:用户改 B超/产检 检查日期时 occurredAt 跟随重生成、否则保留,合理放宽 Q2 原"保 occurredAt"刚性不变量(id/createdAt 仍 existing,核心不破坏)。

**裁定**:全部 10 笔通过,可 push 上 origin。
