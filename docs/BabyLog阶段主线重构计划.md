...
当前真实家庭使用场景以孕期为主，预产期为 2026-09-16。阶段一主线 MUST 按以下顺序推进：

| 顺序 | 主线 | 当前判断 |
|---|---|---|
| 1 | 孕期记录地基 | 首登/档案、阶段投影、孕妈指标、B 超字段、胎儿成长曲线、胎动计数器、长文本语音友好输入 |
| 2 | B 超单多模态识别 | 拍照或选图后调用用户自带多模态模型 API，生成候选 B 超字段，人工确认后入库；优先于出生后日视图精修 |
| 3 | 数据安全门禁 | 导入覆盖确认、profile 备份恢复、未知事件安全、写入后台化、附件唯一文件名 |
| 4 | 家庭共享 MVP | PocketBase + 服务器地址 + 家庭密钥；只同步确认后的结构化记录和附件，不同步用户大模型 API Key |
| 5 | 出生后日视图 | 9 月前完成骨架即可；完整 24 小时轴、提醒、育儿统计可后置到临近出生前 |
| 6 | 视觉精修 | 在主线逻辑稳定后集中做，不与数据/识别功能混提交 |

“用户自带大模型 API Key”属于本机智能输入能力，MUST 与家庭同步解耦。语音文本智能解析和 B 超 OCR SHOULD 共用同一套本机模型配置，但二者都必须以候选确认作为入库前置。
...
| D-24 | 智能输入使用用户自带 Key | 大模型 API Key MUST 由用户本机手动配置；MUST NOT 内置开发者 Key；MUST NOT 同步、备份或写日志 |
| D-25 | OCR/LLM 只生成候选 | B 超单 OCR 和文本智能解析 MUST 只生成候选记录；用户逐字段确认后才能入库 |
...
### R-10 智能输入和 B 超单多模态识别

| 要求 | 规则 |
|---|---|
| R-10.1 | 长文本输入字段 MUST 使用标准多行 Compose `TextField`；系统键盘语音仅作为手动输入 fallback |
| R-10.2 | 语音 v1 MUST 使用单一全局入口：App 内按住录音 -> 云端专业 STT -> 可编辑转写文本 -> LLM 结构化候选 -> 对应表单预填 -> 用户手动确认入库 |
| R-10.2a | 语音 v1 MUST 申请 `RECORD_AUDIO` 并实现权限拒绝降级；权限被拒绝、离线、未配置或 STT 失败时，手动文本输入和手动表单路径必须继续可用 |
| R-10.2b | 语音音频 MUST 只在用户主动按住录音并触发 STT 时发送给用户配置的服务商；MUST NOT 进入备份、同步、日志或事件 payload |
| R-10.3 | B 超单识别 MUST 从已拍照或已选图的 B 超附件触发，MUST NOT 后台自动上传图片 |
...
| R-10.7 | B 超单识别 v1 MUST 自动生成检查日期、BPD、HC、AC、FL、明确 EFW，以及报告明确写出的胎心率、AFI/最大羊水池、胎盘位置/分级、胎位、胎儿个数、胎动、脐带插入处、宫颈管、CRL/NT、脐动脉 S/D/PI/RI 候选；孕周 MUST 由用户根据报告手动填写或由预产期计算，不得由模型识别或推断；侧脑室/后角宽、鼻骨、四腔心、肢体可及等结构筛查文字 MUST 留在 `rawText`/`warnings` 或手动录入，不得自动映射到生长/羊水/NT 字段 |
...
| AC-31 | 长文本备注、产检结论、医生建议类字段支持多行输入；语音录入统一走全局入口，系统键盘语音仅作为手动文本 fallback |
| AC-32 | B 超单识别未配置模型时不影响拍照、选图、手动保存 B 超 |
| AC-33 | B 超单识别成功后展示生长指标和公共信息候选字段；未点击确认前不写入事件库，结构筛查长文本不得自动映射为生长/羊水/NT 字段 |
