...
**栗子吉祥物 v2(1 张)**：现 splash 的 `chestnut_mascot.png` 风格升级为**纯物件型卡哇伊扁平贴纸**——经典栗子轮廓(圆润棕色上半 + 米色下半)、细白贴纸描边 + 柔和阴影、米色下半保持两色干净不加颗粒/纹理、**透明背景**。

> **用户决策(2026-05-20):不拟人。** Codex imagegen 在拟人脸上质量不稳,易出"诡异谷脸"。栗子为**纯物件型贴纸**,**禁用**任何面部特征(点眼/腮红/嘴/表情/手脚/拟人化轮廓);只靠形状/色板/描边卖萌。
