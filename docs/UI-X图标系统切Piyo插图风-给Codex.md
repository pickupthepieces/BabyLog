...
**栗子吉祥物 v2(1 张)**：现 splash 的 `chestnut_mascot.png` 风格升级——经典栗子轮廓(圆润棕色上半 + 米色下半)、细白贴纸描边 + 柔和阴影、米色下半保持两色干净不加颗粒/纹理、**透明背景**;**允许简单拟人**(两个干净点眼 + 柔和腮红 + 小小嘴弯),但只在生成质量到位时保留——质量不达即退回纯物件型。

> **用户决策(2026-05-20 修订):栗子可拟人,5 张快捷图标禁拟人。**
> - 栗子已有合格生成版(简单点眼/克制腮红/小嘴弯),保留;若 imagegen 出"诡异谷脸"则退回纯物件型。
> - **5 张快捷贴纸(B超/产检/胎动/宫缩/孕妈指标)仍强制纯物件型**:禁面部/手脚/拟人轮廓——给夹板/天平/秤加脸会很蠢。
